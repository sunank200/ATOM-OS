...
int strcasecmp(const char *dest, const char *src)
{
	...
	if(dest==src)
		return 0;
	while((toupper(*(dest+i))==toupper(*(src+i)))&&(*(dest+i)!=0)&&(*(src+i)!=0))
		i++;
	return toupper(*(dest+i))-toupper(*(src+i));
}

int strncasecmp(const char *dest, const char *src, unsigned int n)
{
	int i=0;
	if(dest==src)
		return 0;
	while((i < (n-1)) && (toupper(*(dest+i))==toupper(*(src+i)))&&(*(dest+i)!=0)&&(*(src+i)!=0))
		i++;
	return toupper(*(dest+i))-toupper(*(src+i));
}
