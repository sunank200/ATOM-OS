                 ...
                 Bit32u dword1, Bit32u dword2)
{
  ...
  if (v8086_mode()) {
    ...
    if ( (raw_cs_selector & 0xfffc) != 0 ) {
      ...
      {
	char str[160];
	sprintf(str,"task: v : %d, sel: %d, ty: %x, dw1 : %x, dw2 : %x val : %d, ind : %d\n ",cs_descriptor.valid, raw_cs_selector, cs_descriptor.type, dword1, dword2,cs_selector.value,cs_selector.index);
	BX_INFO((str)); 
