...
void find_path_from(int mytid, int memb_of[], int locally_bestpath[], int sub_path[], int cur_city, int cost, int *globalbound, int *solution_task);
...
int locally_bestpath[MAXCITY];		// Best complete tour remembered so far
	...
	for (i = mytid; i < nedges; i = i + ntasks)
	{
		...
		find_path_from(mytid, memb_of, locally_bestpath, sub_path, 1, edge_cost, &globalbound, &solution_task);
...
//			printf(" Including ....\n");
			...
			if (cur_city == ncities-1) 
			{
				...
				if (cost < *globalbound)
				{
					*globalbound = cost;
					*solution_task = mytid;
					printf("New global bound found : %d, %d\n",mytid, cost);
					tsendto(-1, (char *)globalbound, sizeof(int), 0, 0, 1);
				}
...
//				printf("Trying other alternatives ...\n");
			...
			{ 
				// When cur_city == ncities - 2
				// Recieve global bounds if any.
				if (cur_city == ncities - 2)
				{
					recvfrom = -2; // MPI_ANY_SOURCE
					while (trecvfrom((char *)&recvbound, sizeof(int), &recvfrom, 1) > 0) // some message received
					{
						// Update local value
						if (recvfrom < *globalbound)
						{
							*globalbound = recvbound;
							*solution_task = recvfrom;
							printf("New global bound received from : %d, %d\n",recvfrom, recvbound);
						}
				...
				find_path_from(mytid, memb_of, locally_bestpath,sub_path, cur_city + 1, cost, globalbound, solution_task);
